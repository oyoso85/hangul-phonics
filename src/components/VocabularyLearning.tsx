...
import { vocabularyAudioPath } from '../utils/audio';
...
export default function VocabularyLearning() {
  ...
  useEffect(() => {
    if (current) {
      play(vocabularyAudioPath(vocabCategory, current.id));
    }
    return () => stop();
  }, [currentIndex]);
  ...
  const handleReplay = () => {
    if (current) play(vocabularyAudioPath(vocabCategory, current.id));
  };
