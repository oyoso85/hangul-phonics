...
import { generateFirstSoundQuestions, getSubjectLabel } from '../../utils/quiz';
import { playAudio, stopAudio } from '../../utils/audio';
import { playCorrectSound, playWrongSound } from '../../utils/soundEffects';

export default function QuizFirstSound() {
  ...
  useEffect(() => {
    if (subject) {
      setQuestions(generateFirstSoundQuestions(subject, 5));
    }
    return () => stopAudio();
  }, [subject]);
      ...
      setTimeout(() => {
        if (currentIndex + 1 < questions.length) {
          ...
          navigate(`/quiz-result`, {
            state: { score: score + 1, total: questions.length, backTo: `/quiz/first-sound/categories` },
          });
        }
      }, 1200);
    } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 py-8">
      ...
      <div className="my-8 flex flex-col items-center gap-3">
        <div className="w-28 h-28 rounded-3xl bg-white shadow-xl flex items-center justify-center text-5xl animate-bounce-in animate-fill-both">
          {currentQ.displayEmoji}
        </div>
        <p className="text-2xl font-bold text-foreground">{currentQ.displayWord}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 w-full max-w-sm">
        {currentQ.choices.map((choice, idx) => (
          <button
            ...
            disabled={answered}
            className={`py-5 rounded-2xl shadow-md text-2xl font-bold transition-all duration-200 ${
      ...
      <button
        onClick={() => navigate(`/quiz/first-sound/categories`)}
